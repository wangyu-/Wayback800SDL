...
const int CYCLES_NMI =spdc1016freq/2;
long long my_nmi_cycles=CYCLES_NMI;
void EmulatorThread::do_run(uint64_t target_cycle)
{
        ...
        while (totalcycle < target_cycle) {
            ...
            // in CC800 hardware, NMI is generated by 32.768k crystal, but spdc1016 use RC oscillator so cycle/NMI can be mismatched.
            unsigned int dummynow = GetTickCount();
            if (dummynow - nmistart >= 500 && !perdictable_nmi) {
                nmistart += 500;
#endif
                //g_nmi = 0; // next CpuExecute will execute two instructions
                gThreadFlags |= 0x08; // Add NMIFlag
            ...
            if (totalcycle >= my_nmi_cycles && perdictable_nmi) {
			    my_nmi_cycles += CYCLES_NMI;
                gThreadFlags |= 0x08; // Add NMIFlag
            }
            ...
            my_tick++;
            if(debug_pc){
                printf("tick=%lld ",my_tick /*, reg_pc*/);
